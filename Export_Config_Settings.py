...
def config_translate(row: pd.Series, eel: bool):
    ...
    if team1_settings is not None and team2_settings is None:  # layer|exclusions_team1|
        ...
        team1_settings is None and team2_settings is not None
    ):  # layer||exclusions_team2
        output_string = f"{layer}||{team2_settings}"
    elif (
        team1_settings is not None and team1_settings == team2_settings
    ):  # layer|exclsionons_both
        output_string = f"{layer}|{team1_settings}"
    elif (
        team1_settings is not None and team2_settings is not None
    ):  # layer|exclusions_team1|exclusions_team2
...
def settings_check(row: pd.Series):
    ...
    layer_df[["Faction", "Unit"]] = (
        layer_df["index"].str.rsplit("_", n=2, expand=True).iloc[:, :2]
    )
    ...
    layer_df = layer_df[
        ~layer_df["Faction"].isin(factions) & ~layer_df["Unit"].isin(units)
    ]
