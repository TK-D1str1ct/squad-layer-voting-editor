...
def config_to_dict(settings_list: list):
    ...
    for layer in settings_list:
        ...
        if excluded_layer:
            layer = layer.lstrip(
                "//"
            ).strip()  # If it is excluded strip away the "// " TODO make sure it works without space or maybe not

        parts = layer.split(
            "|"
        )  # Split the config line into the different exclusion parts based on the "|"
        ...
        if len(parts) == 1:  # If only layer name is given --> pass
            ...
            len(parts) == 2
        ):  # If there is only 1 '|' separator: layer|exclsionons_both
            excl = parts[1].split()
            if (
                parts[1] == ""
            ):  # Redundancy check in case there is no exclusions provided
                pass
            else:
                team1, team2 = excl, excl
        elif len(parts) == 3:  # If there is 2 '|' separators
            if (
                parts[1] and not parts[2]
            ):  # If the separators have an exclusions in between but not after the last separator: layer|exclusion_team1|
                team1 = parts[1].split()
            elif (
                not parts[1] and parts[2]
            ):  # If the separators dont have an exclusions in between but do after the last separator: layer||exclusion_team2
                team2 = parts[2].split()
...
def apply_config_settings(
    ...
    if layer_config_settings is None:  # Check if the layer exists in the .cfg file
        missing_layers.append(
            layer
        )  # If the layer does not exist it will be added to a warning list and no exclusions at all will be implemented
        ...
        layer_settings = get_config_settings(
            layer_settings, layer_config_settings["team1"], 1
        )
        layer_settings = get_config_settings(
            layer_settings, layer_config_settings["team2"], 2
        )
...
def get_config_settings(
    ...
    for exclusions in team_config_settings:
        ...
        if exclusions.startswith("+"):
            unit = exclusions
            mask = (
                layer_settings.index.str.contains(f"_{unit}_")
                & layer_settings.index.str.contains(f"_{team}")
                & layer_settings.notna()
            )
            ...
            mask = (
                layer_settings.index.str.startswith(f"{faction}_{unit}")
                & layer_settings.index.str.contains(f"_{team}")
                & layer_settings.notna()
            )
            layer_settings[mask] = True
            ...
            faction = exclusions
            mask = (
                layer_settings.index.str.startswith(f"{faction}_")
                & layer_settings.index.str.contains(f"_{team}")
                & layer_settings.notna()
            )
            layer_settings[mask] = True
...
def upload_cfg_to_df(
    ...
    if isinstance(cfg_input, str):
        # Manual paste: split into lines
        settings_list = [
            line.strip() for line in cfg_input.splitlines() if line.strip()
        ]
    else:
        # File upload: read lines as bytes and decode
        settings_list = [
            line.decode("utf-8").strip() for line in cfg_input if line.strip()
        ]

    LFUT_import_df = LFUT_empty_df.apply(
        apply_config_settings,
        axis=1,
        args=(
            config_to_dict(settings_list),
            missing_layers,
            eml,
            eal,
            ial,
