...
def create_info_df_JSON(data: dict, columns: list, filter: list, key: str):
    if key == "Maps":
        ...
        info_df = info_df[
            ~info_df[columns[1]].isin(filter_faction)
        ]  # Filter out CIV faction
...
def create_faction_variables_JSON(info_df: pd.DataFrame, columns: list):
    ...
    Alliances = (
        info_df.groupby(columns[2])[columns[1]]
        .unique()
        .apply(lambda x: sorted(x))
        .to_dict()
    )
    Factions = [item for values in Alliances.values() for item in values]
    Faction_Unit_Team_All = [
        f"{x}_{y}_{z}" for z in Teams for x in Factions for y in Units
    ]
...
def LFUT_JSON(
    info_df: pd.DataFrame, LFUT_empty: pd.DataFrame, columns: list, units: list
):
...
Alliances, Factions, Units, Teams, Faction_Unit_Team_All = (
    create_faction_variables_JSON(factions_info, faction_columns)
)
