...
def map_page():
    ...
    selected_map = st.selectbox("Choose a Map filter:", au.maps, key="map_filter")
    ...
    au.build_bottom_nav(map_page, df=st.session_state.df, table_1_df=au.state_maping(team_1_table, au.reverse_state_map), table_2_df=au.state_maping(team_2_table, au.reverse_state_map), filter=st.session_state.map_filter)